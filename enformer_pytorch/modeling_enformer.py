...
class ConvNextBlock(nn.Module):
    def __init__(self, dim, dim_out=None, kernel_size=7, ff_mult=2):
        super().__init__()
...
class Enformer(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.Sequential(Rearrange("b d n -> b n d"), *transformer)
        ...
        self._heads = nn.ModuleDict(
            map_values(
                lambda features: nn.Sequential(
                    nn.Linear(twice_dim, features), nn.Softplus()
                ),
                config.output_heads,
            )
        )
    ...
    def trunk_checkpointed(self, x):
        ...
        x = checkpoint_sequential(
            nn.Sequential(*transformer_blocks), len(transformer_blocks), x
        )
